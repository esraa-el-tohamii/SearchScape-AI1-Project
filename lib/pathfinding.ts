...
export function dls(
  ...
  function dlsRecursive(
    ...
    if (visitedDepths.has(key) && visitedDepths.get(key)! <= depth) {
      return false;
    }
    visitedDepths.set(key, depth);
    current.visitOrder = visitOrder++;
    ...
    for (const neighbor of getNeighbors(grid, current, mode)) {
      // Create a temporary reference for parent
      const originalParent = neighbor.parent;
      neighbor.parent = current;
      if (dlsRecursive(neighbor, depth + 1)) {
        return true;
      }
      // Revert parent if not part of the found path
      if (!foundPath.length) {
