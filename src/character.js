...
export class CharacterController {
    ...
    place(x, y) {
        ...
        this.el.style.cssText = `
            ...
            left: ${p.cx}px;
            top: ${p.cy}px;
            width: ${p.w * d.placeScale}px;
            height: ${p.h * d.placeScale}px;
            transition: none;
            transform: translate(-50%, -50%);
            background-image: url('${base}/${c.idleFront}');
        `;
        this._setState('idle');
    }
        ...
        return new Promise(resolve => {
            ...
            if (this.currentPos) {
                ...
                if (dx < 0) {
                    ...
                    bgImage = this.stepToggle ? c.moveUpA : c.moveUpB;
                } else if (dy > 0) {
                    bgImage = this.stepToggle ? c.moveDownA : c.moveDownB;
                }
                
                if (bgImage) {
                    this.el.style.backgroundImage = `url('${base}/${bgImage}')`;
                }
                this._setState('walking');
            }

            const d = SPRITE_CONFIG.dimensions;
            const charW = p.w * d.walkScale;
            const charH = p.h * d.walkScale;
